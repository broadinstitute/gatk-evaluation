...
workflow THFromCoverageWorkflow {

    ##################################
    #### required basic arguments ####
    ##################################
    File entity_id
    File tumor_read_counts
    File tumor_allelic_counts
    ...
    Int disk_pad = 20 + gatk4_override_size + select_first([emergency_extra_disk, 0])

    Int denoise_read_counts_tumor_disk = read_count_pon_size + disk_pad
    call DenoiseReadCounts as DenoiseReadCountsTumor {
        input:
            entity_id = entity_id,
            read_counts = tumor_read_counts,
    ...
    Int model_segments_tumor_disk = ceil(size(DenoiseReadCountsTumor.denoised_copy_ratios, "GB")) + disk_pad
    call ModelSegments as ModelSegmentsTumor {
        input:
            entity_id = entity_id,
            denoised_copy_ratios = DenoiseReadCountsTumor.denoised_copy_ratios,
            allelic_counts = tumor_allelic_counts,
    ...
    Int plot_tumor_disk = ceil(size(DenoiseReadCountsTumor.standardized_copy_ratios, "GB")) + ceil(size(DenoiseReadCountsTumor.denoised_copy_ratios, "GB")) + ceil(size(ModelSegmentsTumor.het_allelic_counts, "GB")) + ceil(size(ModelSegmentsTumor.modeled_segments, "GB")) + disk_pad
    ...
    output {
        ...
        Array[File] tumor_heterogeneity_output = TumorHeterogeneity.tumor_heterogeneity_output
    }
}
...
task TumorHeterogeneity {
    ...
    command <<<
        ...
        conda env create -n gatk-th -f ${environment_yml}
        source activate gatk-th

        python ${python_script} \
            --modeled_segments ${modeled_segments} \
            --output_path ${output_dir_} \
            --output_prefix ${entity_id}
    >>>
    ...
    output {
        Array[File] tumor_heterogeneity_output = glob("${output_dir_}/*")
    }
}
