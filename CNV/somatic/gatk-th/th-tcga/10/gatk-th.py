...
def generate_label_ordered_product_states_and_log_prior(discrete_prior_config):
    ...
    unnorm_label_ordered_product_state_prior_li *= np.product(1. - (1. - allelic_copy_number_change_prior_prob) * ~np.all(label_ordered_allelic_copy_number_product_states_lij == normal_allelic_copy_number_state, axis=-1), axis=-1)[:, np.newaxis]
...
continuous_prior_config = ContinuousPriorConfig(
    ...
    subclonal_cancer_cell_fraction_alpha = 1E-1,
    purity_a = 1.,
    purity_b = 10.,
    cr_norm_s = 0.1,
    cr_norm_scale = 2.,
