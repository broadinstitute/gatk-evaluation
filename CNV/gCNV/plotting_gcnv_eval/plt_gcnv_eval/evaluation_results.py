...
class EvaluationResult:
    ...
    def get_false_positive_rate(self, filter_to_use: str):
        fp_bases = self.confusion_matrix_pd.loc[filter_to_use, ConfusionType.FP_BASES.name]
    ...
    def write_area_under_roc_to_file(self, file: str, filters: List[CallsetFilter]):
        ...
        sensitivity_values = [self.get_recall(str(f)) for f in filters]
        false_positive_rate_values = [self.get_false_positive_rate(str(f)) for f in filters]
